  ...
  return (
    ...
    onClick={stateLogin ? () => handleRedirect(card.redirect) : () => handleSignIn() }
     >
      ...
      {isShown && (
            <button className="flex flex-row text-left h-full text-[#ff00f0] leading-6 rounded-3xl justify-center shadow-s font-open-sans font-semibold text-base mt-4 leading buttonsNFTSection mx-auto duration-100"
            >
              {card.sub}
              <img
